...
int results::analyze( ofstream& lgf, parameters& parz ){//averaging over runs, complicated by NA
  ...
  if(  !light && n > 1 ){	// heavy recording too
    ...
    // double ones...
    // non-zero TB deaths
    collapse_result_averager( TBdeathavA, TBdeaths );
    ...
    // non-zero notifs
    collapse_result_averager( TBpropRetx, tbnotes);
    ...
    for( unsigned int ui = 0; ui < ibmrx.size(); ++ui){
      collapse_result_averager( ibmrx[ui] );
    }
    for( unsigned int ui = 0; ui < ibmrxcp.size(); ++ui){
      collapse_result_averager( ibmrxcp[ui] );
    }

    collapse_result_averager( ibmnbcrx );
    collapse_result_averager( ibmnintrx );
    collapse_result_averager( rxcat );    // cat 2 treatments counter
...
int results::write( ofstream& lgf, parameters& parz, time_t& writetime ){
  ...
  if( !light ){
    ...
    MAsmooth( TBdeaths[0], 3 );	// TB deaths
    MAsmooth( HIVdeaths[0], 3 );	// HIV deaths
    MAsmooth( TBHIVdeaths[0], 3 );	// TB/HIV deaths
	...
	res_strm<<"\t";
	if(  TBdeathavA[0].at(i) > 0 ){ res_strm << TBdeathavA[0].at(i); } else { res_strm << "NaN"; }
	res_strm<<"\t";
	if( TBpropRetx[0].at(i) > 0 ){ res_strm << TBpropRetx[0].at(i); } else {res_strm << "NaN"; }
	res_strm << endl;
      }
      res_strm.close();
    ...
    lgf << "opening results file 6 ..." << endl;
    res_strm.open(  (parz.wkdir + "results6.dat").c_str() );
    if(!res_strm){lgf << "results5 file failed to open..." << endl;} else {
      lgf << "results5 file opened..." << endl;
      for(int i = 0; i < notime; ++i){//treatment counts
  	res_strm <<  time.at(i) << "\t";
        for( unsigned int ui = 0; ui < ibmrx.size(); ++ui ){
          res_strm << ibmrx[ui][0].at(i) << "\t";
        }
    	for( unsigned int ui = 0; ui < ibmrxcp.size(); ++ui ){
          res_strm << ibmrxcp[ui][0].at(i) << "\t";
        }
   	res_strm  << ibmnbcrx[0].at(i) << "\t" << ibmnintrx[0].at(i);
  	res_strm << endl;
      }
      res_strm.close();
      lgf << "closing results6..." << endl;
    }

    lgf << "opening results file 7 ..." << endl;
    res_strm.open(  (parz.wkdir + "results7.dat").c_str() );
    if(!res_strm){lgf << "results7 file failed to open..." << endl;} else {
      lgf << "results7 file opened..." << endl;
      for(int i = 0; i < notime; ++i){//For DALY calcs
      N = ibmN[0].at(i);
  	res_strm <<  time.at(i) << "\t";
  	res_strm << strIn[0].at(i) <<"\t"<< strIp[0].at(i) <<"\t"<< strIa[0].at(i)<<"\t";
  	res_strm << TBdeaths[0].at(i) <<"\t";
	if(  TBdeathavA[0].at(i) > 0 ){ res_strm << TBdeathavA[0].at(i); } else { res_strm << "NaN"; }
  	res_strm << "\t";
        for( unsigned int ui = 0; ui < ibmrx.size(); ++ui ){
          res_strm << ibmrx[ui][0].at(i) << "\t";
