...
int parameters::read( ofstream& lgf, string& wdir ){//reading parameters in
  ...
  if(!parfile){
    ...
    // complex treatment outcomes
    getline( parfile, line);//comments
    getline( parfile, line);
    ss.clear();
    ss.str(line);
    ss >> rxFN0 >> rxFN1;
    ss >> rxt0 >> rxt1 >> rxstart >> rxrate >> mdrprop0 >> mdrprop1 >> mdrt0 >> mdrt1 >> mdrretxOR;
    // BC reg, INT reg, BC retx, INT retx
    lgf << "treatment outcomes:" << endl;
    lgf << "\tparz.rxFN0 = " << rxFN0 << endl;
    lgf << "\tparz.rxFN1 = " << rxFN1 << endl;
    lgf << "\tparz.rxt0 = " << rxt0 << endl;
    lgf << "\tparz.rxt1 = " << rxt1 << endl;
    lgf << "\tparz.rxstart = " << rxstart << endl;
    lgf << "\tparz.rxrate = " << rxrate << endl;
    lgf << "\tparz.mdrprop0 = " << mdrprop0 << endl;
    lgf << "\tparz.mdrprop1 = " << mdrprop1 << endl;
    lgf << "\tparz.mdrt0 = " << mdrt0  << endl;
    lgf << "\tparz.mdrt1 = " << mdrt1 << endl;
    lgf << "\tparz.mdrretxOR = " << mdrretxOR << endl;
...
//   //DEBUG!
  ...
  // complex treatment outcomes
  if ( rxFN0 != "none" ){      	// yes, using treatment outcomes data
    vector< vector< double > > tempmat;
    cout << "reading the treatment outcomes!"<< endl;
    ostringstream convert;      // for converting u-ints to strings
    for( unsigned int interv = 0; interv < 2; ++interv ){
      for( unsigned int retx = 0; retx < 2; ++retx ){
        for( unsigned int hivstatus = 0; hivstatus < 3; ++hivstatus ){
          for( unsigned int mdr = 0; mdr < 2; ++mdr ){
            // 1st time files
            convert.str("");      // clear
            convert << interv << retx << hivstatus << mdr;
            fname = rxFN0 + convert.str();
            popfile.open ( fname.c_str() );
            tempmat.clear();
            if ( popfile.is_open() ){            // reading in
              while( getline( popfile, line ) ){ // month by month
                ss.clear();
                tempvec.clear();
                ss.str(line);
                while ( ss >> tempa ){tempvec.push_back( tempa );}
              ...
              rxdata0.push_back( tempmat );
              popfile.close();
              lgf << "treatment outcome0: " << interv << retx << hivstatus << mdr <<" data read..." << endl;
            } else {
              cerr << "**treatment file0:" << interv << retx << hivstatus << mdr <<" not found!**" << endl;
            }
            // second time files
            fname = rxFN1 + convert.str();
            popfile.open ( fname.c_str() );
            tempmat.clear();
            if ( popfile.is_open() ){            // reading in
              while( getline( popfile, line ) ){ // month by month
              ...
              rxdata1.push_back( tempmat );
              popfile.close();
              lgf << "treatment outcome1: " << interv << retx << hivstatus << mdr <<" data read..." << endl;
            } else {
              cerr << "**treatment file1:" << interv << retx << hivstatus << mdr <<" not found!**" << endl;
            }
          } // MDR loop
        } // HIV loop
      }   // retx loop
    ...
    // checks 0 
    for( unsigned int kk = 0; kk < rxdata0.size(); ++kk){
      if ( rxdata0[kk].at(0).size() != 4){lgf << "**expecting 4 columns in rxdata0! "<<kk<<"**" << endl;} 
      for(unsigned int ii=0; ii<rxdata0[kk].size(); ++ii){
	if( rxdata0[kk].at(ii).at( rxdata0[kk].at(ii).size()-1 ) > 1 ){
	  lgf << "**error: probs must be <=1 in treatment outcome data!**" << endl;
	  lgf << "position "<<ii<<"," << kk << endl;
	  cerr << "error: probs must be <=1 in treatment outcome  data!" << endl;
	  cerr << "position "<<ii<<"," << kk << endl;
	}
	for(unsigned int jj=2; jj<rxdata0[kk].at(ii).size(); ++jj){
	  if( rxdata0[kk].at(ii).at(jj) < rxdata0[kk].at(ii).at(jj-1)  ){
	    lgf << "**error: expecting cumulative probs in treatment outcome data!**" << endl;
	    lgf << "position "<<ii<<","<<jj<<"," << kk << endl;
	    cerr << "error: expecting cumulative probs in treatment outcome  data!" << endl;
	    cerr << "position "<<ii<<","<<jj<<"," << kk << endl;
	  }
	}

        // // debug check for read in
	// for(unsigned int jj=0; jj<rxdata0[kk].at(ii).size(); ++jj){
	//   cout <<kk<<": "<<ii<<","<<jj<<","<<rxdata0[kk].at(ii).at(jj)<<endl;
	// }
    ...
    // checks 1 
    for( unsigned int kk = 0; kk < rxdata1.size(); ++kk){
      if ( rxdata1[kk].at(0).size() != 4){lgf << "**expecting 4 columns in rxdata1! "<<kk<<"**" << endl;} 
      for(unsigned int ii=0; ii<rxdata1[kk].size(); ++ii){
        if( rxdata1[kk].at(ii).at( rxdata1[kk].at(ii).size()-1 ) > 1 ){
          lgf << "**error: probs must be <=1 in treatment outcome data!**" << endl;
          lgf << "position "<<ii<<"," << kk << endl;
          cerr << "error: probs must be <=1 in treatment outcome  data!" << endl;
          cerr << "position "<<ii<<"," << kk << endl;
        }
        for(unsigned int jj=2; jj<rxdata1[kk].at(ii).size(); ++jj){
          if( rxdata1[kk].at(ii).at(jj) < rxdata1[kk].at(ii).at(jj-1)  ){
            lgf << "**error: expecting cumulative probs in treatment outcome data!**" << endl;
            lgf << "position "<<ii<<","<<jj<<"," << kk << endl;
            cerr << "error: expecting cumulative probs in treatment outcome  data!" << endl;
            cerr << "position "<<ii<<","<<jj<<"," << kk << endl;
        ...
        // // debug check for read in
        // for(unsigned int jj=0; jj<rxdata1[kk].at(ii).size(); ++jj){
        //   cout <<kk<<": "<<ii<<","<<jj<<","<<rxdata1[kk].at(ii).at(jj)<<endl;
        // }
    ...
    // checks 1 vs 2
    if( rxdata0.size() != rxdata1.size() ){lgf << "** rxdata size mismatch! **" << rxdata0.size() << rxdata1.size() << endl; exit(-1);}
    for( unsigned int kk = 0; kk < rxdata1.size(); ++kk){
      if( rxdata0[kk].size() != rxdata1[kk].size() ){
        lgf << "** rxdata size mismatch! **" << kk << rxdata0[kk].size() << rxdata1[kk].size() << endl; exit(-1);
      }
      for(unsigned int ii=0; ii<rxdata1[kk].size(); ++ii){
        if( rxdata0[kk].at(ii).at(0) != rxdata1[kk].at(ii).at(0) ){
          lgf << "** rxdata stage time mismatch! **" << endl; exit(-1);
        }
      }
    } // end checks
    

  } // end read in of treatment outcome data
...
double parameters::h1549( double yr ){
  ...
  if( hivincF ){		      // using file-read spline-fit data
    if ( yr < h1549_yrz.front() || yr > h1549_yrz.back() ){
      //ans = 0;
      ans = (yr > h1549_yrz.back() ? h1549_dat.back() / 100 : 0) ;
    } else {
...
//////////////////////////////////////////////////////////////////////////////////////////////
double parameters::rxINTcov( void ){
  // coverage of new regimens
  double ans = 0;
  if( time > rxstart ){
    ans = rxrate * (time - rxstart);
    ans /= (1+ans);
  }
  return ans;
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
//double parameters::rxprob( int reg, int rxst, int n ){
//  // interpolating the probabilities for rxdata0 and rxdata1
//  double ans = rxdata0[reg].at( rxst ).at(n);
//  if( rxt1 > rxt0 ){          // putting second time <= 1st turns off increase
//    double ans1 = rxdata1[reg].at( rxst ).at(n);
//    if( time > rxt0 && time < rxt1 ){
//      ans += (ans1-ans) * (time - rxt0) / (rxt1 - rxt0);
//    } else { ans = ans1; }
//  }
//  return ans;
//}
...
//////////////////////////////////////////////////////////////////////////////////////////////
//double parameters::rxMDRprop( int retx ){
//  // proportion MDR at treatment
//  double ans = mdrprop0;
//  if( mdrt1 > mdrt0 ){          // putting second time <= 1st turns off increase
//    if( time > mdrt0 && time < mdrt1 ){
//      ans += (mdrprop1-mdrprop0) * (time - mdrt0) / (mdrt1 - mdrt0);
//    } else { ans = mdrprop1; }
//  }
//  if( retx ){ans = ans * mdrretxOR / (1+(mdrretxOR-1) * ans);}
//  return ans;
//}
...
double parameters::rxprob( int reg, int rxst, int n ){
  // interpolating the probabilities for rxdata0 and rxdata1
  double ans = rxdata0[reg].at( rxst ).at(n);
  if( time > rxt0 ){          // putting second time <= 1st turns off increase
    double ans1 = rxdata1[reg].at( rxst ).at(n);
    if( time < rxt1 ){
      ans += (ans1-ans) * (time - rxt0) / (rxt1 - rxt0);
    } else { ans = ans1; }
...
double parameters::rxMDRprop( int retx ){
  // proportion MDR at treatment
  double ans = mdrprop0;
  if( time > mdrt0 ){          // putting second time <= 1st turns off increase
    if( time < mdrt1 ){
      ans += (mdrprop1-mdrprop0) * (time - mdrt0) / (mdrt1 - mdrt0);
    } else { ans = mdrprop1; }
  }
