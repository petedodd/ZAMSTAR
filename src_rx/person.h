...
public:
  ...
  int rxst, rxb4, rxb4l, rxb4t, rxtype;// treatment stage, previous rx?, type of rx eligible for
  double age, tau, taumax, taumax0, taustart, sigma, Rxtime, 
    tsi, tbout, cd4, cd42, cd4st;//hiv, tbinfection, tbactive (tboutcome)
  ...
  int isD, isrD, mdr;//TB disease (resistant), this last flag is for the rxoutcome stuff
  int isSmr;//smr +
  int isT;//on TB Rx
  int isX;//dead
  int isART;//on ART
  ...
  int phhi, hhart, hhipt, tecf, iptd;//has received hh intervention in the past; on ART from HH; had IPT from HH; touched by real ECF
 ...
 person(unsigned int g = 0,  double a = 23, unsigned int n = 2):
  tbrate(0.0),gender(g), hiv(0), Lhiv(0), hhhiv(0),  dart(0), bbs(0), shiv(0), dshiv(0), hhtb(0), ARTrefusenik(0), 
    acat(0), scat(0), rxst(-1), rxb4(0), rxb4l(0), rxb4t(0), rxtype(0), age(a), tau(0.0), taumax(99), taumax0(99), taustart(-1.0), sigma(0.0), Rxtime(0.0), 
    tsi(0.0), tbout(99), cd4(1000), dTBact(0),
    noevs(n), indhaz(noevs,0),
    is1549(0), isS(1), isL(0),isrL(0), isPI(0),
    isD(0),isrD(0), mdr(0), isSmr(0), isT(0), isX(0), isART(0), isIPT(0), prevTB(0), artdefr(0),
    phhi(0), hhart(0), hhipt(0), tecf(0), iptd(0), aluck(-1),fast(0), tonipt(-1), artstrate(0), GL(-1), CD4cat(-1) {}
