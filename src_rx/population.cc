...
double population::update_hazards( ofstream& lgf, parameters& parz ){//hazard and foi updater
  ...
  if( !parz.DHflag ){		    // HIV not dynamic
    if( !parz.hivmac ){			    //!parz.hivmac ){ // wonder if this is cause HIV overshoot? debug
...
int population::counts( ofstream& lgf, parameters& parz ){//keeps tabs of population counts
  ...
  nophhi = 0;nohhipt = 0;nohhart = 0; notecf = 0; 
  size18 = 0; prev18 = 0; prevSP18 = 0; prevT18 = 0; prevTH18=0;        // adult population
  prev_rx.assign(24,0);
  prev_rx_cp.assign(24,0);
  nbc_rx = 0; nint_rx = 0; // numbers of treatments counters
  rL = 0;		       // resistant
  ARTnum=0; 			// ART number
  ...
  for( unsigned int uu = 0; uu < v.size(); ++uu){
	...
	if( v.at(uu).isD ){ ++Dp;}
	if( v.at(uu).isT ){ ++Tp;
          //! counters for prevalence of treatment (each type) and also starting treatment counters
          ++prev_rx.at( v.at(uu).rxtype );
		if( v.at(uu).Rxtime > 2 ){ ++prev_rx_cp.at( v.at(uu).rxtype );}

          if( v.at(uu).Rxtime < parz.dt ){ // I haven't thought this through!
            if( v.at(uu).rxtype < 12 ){
              ++nbc_rx;
            } else { ++nint_rx; }
          }
        }
...
unsigned int population::setup_weights(ofstream& lgf, parameters& parz, int kappa, map<int,int>& ghk, vector< double >& weights, vector< unsigned int>& where, unsigned int& noe, unsigned int& num, const gsl_rng *r ){
	  ...
	  for( unsigned int ui = 0; ui < hh.size(); ++ui ){
	    ...
	    if( nopeo < a_weight.size() ){
	      weights.at(k) = a_weight.at( nopeo );
              if( weights.at(k) < 0 ){weights.at(k) = 1e-15;} // erm, this shouldnt be needed...
	      where.at(k) = ui;
	      ++k;			// advance abbreviated counter
	    }
	  }
  ...
  case 11:			// correxion death
	...
	if( over > 0.0005 ){
	  j = auxFloor(  over * size ); // how many extra deaths needed
        ...
        //   // j = 0;lgf <<"warning: population undershoot - correction not possible" << endl;
	//   // lgf << size <<" "<<parz.popsize0<<" "<<relpop <<" "<<parz.rpop() << endl;
        // }

	if( j > 0 ){
	  for ( unsigned int uu = 0; uu < v.size(); ++uu ){
	    if ( !v.at(uu).isX ){//a living ECF naive
	      where.at(k) = uu;
	      weights.at(k) = 1.0; //random death weights
	      ++k;//advance abbreviated counter
	    }
	  }//population loop
	}  // j > 0
  ...
  case -3:	//true ECF
	...
	j = howmany_events( r, k * parz.tECFhaz, parz.dt  );//how many 
      } else {j=0;}
    }
    break;
...
int population::scheduledevents( ofstream& lgf, const gsl_rng *r, bool qlog, parameters& parz, map < string, int >& evk ){//for things going on in events
  ...
  ntbd=0; 			// no TB deaths Rx and undetected
  ...
  ntbretx = 0;			// no detections previously treated
  catII = 0;			// number of catII treatments, no impact on outcome
  iptends = 0;		     	// IPT completions
  


  int bbsstarts = 0;		// starts via bbs for log
  nod = 0; bdcount = 0; avAdeath = 0; tbavaod = 0;
  double tau = 0;
  int j(0);
 ...
 //loop through events -/TB activations, Rx completions, and ART starts, HIV self-knows
  for ( unsigned int ui = 0; ui < v.size(); ++ui ){
	...
	if( ( v.at(ui).hiv && v.at(ui).tonipt > parz.IPTdurnP ) ||
      ...
      if( parz.rxFN0 == "none" ){			    // NB the first one determines behaviour
	if ( v.at(ui).isT && (v.at(ui).Rxtime > parz.RxL)){ // simple treatment outcomes
	    ...
	    ++noc;
	  } // end on rx & due to finish
      } else {			// complex treatment outcomes
	if ( v.at(ui).isT 
	     && (12 * v.at(ui).Rxtime > parz.rxdata0[v.at(ui).rxtype].at( v.at(ui).rxst ).at(0)) ){ 
	  // passed the latest rx stage
	  // def & cure, def & non-cure, die; the rest continue, unless last when def&non-cure
	  // for this few, not worth keying and searching - do by hand
          // there are checks that the data0 & data1 dimensions and times match - but they must!
	  unsigned int reg = v.at(ui).rxtype; // regimen type
	  // outcomes
	  double runif = gsl_rng_uniform(r);
	  if(  runif < parz.rxprob( reg, v.at(ui).rxst, 3 )  ){
	    if(  runif < parz.rxprob( reg, v.at(ui).rxst, 1 )  ){
	      event( ui, 10, parz, r );// def & cure (not at higher risk)
	    } else {
	      if(  runif < parz.rxprob( reg, v.at(ui).rxst, 2 ) ){
		event( ui, 22, parz, r );// def & non-cure
	      } else {
	  ...
	  // if(  runif < parz.rxdata0[reg].at( v.at(ui).rxst ).at(3)  ){
	  //   if(  runif < parz.rxdata0[reg].at( v.at(ui).rxst ).at(1)  ){
	  //     event( ui, 10, parz, r );// def & cure (not at higher risk)
	  //   } else {
	  //     if(  runif < parz.rxdata0[reg].at( v.at(ui).rxst ).at(2)  ){
	  //       event( ui, 22, parz, r );// def & non-cure
	  //     } else {
	  //       event( ui, 6, parz, r );// die
	  //       if( v.at(ui).hiv ){++ntbdh;}
	  ...
	  } else {		// continuation (majority)
	    // continuation
	    ++v.at(ui).rxst;	// update rx stage
	    if( v.at(ui).rxst >= (int)parz.rxdata0[reg].size() ){ // now passed rxL w/o outcome
	      event( ui, 22, parz, r );// def & non-cure
	    }
	  }
	} // end action required
	...
	if( v.at(ui).dTBdet ){
	  ...
	  if( v.at(ui).rxb4 > 0 ){ ++ntbretx; 
	  
	  	if (v.at(ui).rxtype % 2 < 1){  // If even rxtype then not mdr 
	  		if (v.at(ui).isSmr){ // Only get if smear +
	  			if (v.at(ui).rxb4t > 4) {++catII;} // If treated for 5months or more last time
	  			else {if (time - v.at(ui).rxb4l > 0.2 ){++catII;};}  // If treated less than ~2months ago
			}	  	
	  	}
	  } // detected TB that's retx
...
int population::event( unsigned int who, int etype, parameters& parz, const gsl_rng *r ){//event doer
  ...
  if ( etype >= 0){
	...
	j = (int)who;
      ...
      break;
    case 22://treatment failure - like activation but without the counting
	...
	v.at(who).isT = 0;  // no longer on treatment
	
	v.at(who).rxb4l = v.at(who).rxst; 	// Length last treatment
	v.at(who).rxb4t = time;		// Time finished last treatment
	
	v.at(who).rxst = -1;  // no longer on treatment
	...
	if( gsl_rng_uniform(r) < v.at(who).probsmrpos( parz ) ){
	  v.at(who).isSmr = 1;//smear positive
        ...
        if( parz.rxFN0 != "none" ){ // complex Rx outcomes
          if( gsl_rng_uniform(r) < parz.rxMDRprop( v.at(who).rxb4 ) ){
            v.at(who).mdr = 1;//MDR+ for the Rx outcomes
          }
        }
	//destined for death or sc?, or detection
	v.at(who).setTBoutcome( r, parz, time );
	
	if( etype == 2 ){      // don't count failure
	...
	v.at(who).Rxtime = 0;
	v.at(who).rxst = 0;
	if( parz.rxFN0 != "none" ){ // complex regimens
          unsigned int interv(0), retx(0), hivstatus(0), mdrstatus(0);
          if( gsl_rng_uniform(r) < parz.rxINTcov() ){ interv = 1;}
          if( v.at(who).rxb4 ){ retx = 1; }
          if( v.at(who).hiv ){
            if( v.at(who).isART ){
              hivstatus = 2;
            } else { hivstatus = 1; }
          }
          mdrstatus = v.at(who).mdr;
          v.at(who).rxtype = 12 * interv + 6 * retx + 2 * hivstatus + mdrstatus;
	} // end complex regimens
	v.at(who).rxb4 += 1;
       // rxtype 0-23 in order of I,Retx,HIV,MDR loops
	...
	if( parz.hhFLAG ){//Includes coverage!
	  ...
	  for( hmi = hh.at(loc).members.begin(); hmi != hh.at(loc).members.end(); ++hmi ){
		...
		if( time > parz.HHIst && time < parz.HHIet && gsl_rng_uniform(r) < parz.HHIcov ){
		  // BC record
		  v.at(who).phhi = 1;//does this once each loop which is wasteful
		  v.at(hm).phhi = 1; // has had a household intervention
		  ...
		  if( v.at(hm).isD ){//another undetected case - don't recurse
		    ...
		    v.at(who).rxst = 0;
		    if( parz.rxFN0 != "none" ){ // complex regimens
		      if( gsl_rng_uniform(r) < parz.rxINTcov() ){
			if( v.at(who).rxb4 ){
			  v.at(who).rxtype = 3;
			} else {
			  v.at(who).rxtype = 1;
    ...
    case 7:      //Rx completion failure (not real failure) (sched)
    case 5:			// clearance!
      {
	v.at(who).isS = 1;
	v.at(who).isPI = 1;
...
int population::initialinfect( const gsl_rng *r, ofstream& lgf, parameters& parz ){//initial infections
  ...
  while( T0 > 0){//those on Rx
    ...
    if( !v.at(who).isD && !v.at(who).isT ){//not already onRx
      ...
      v.at(who).rxst = auxFloor(v.at(who).Rxtime * 12) ; // number of months
      v.at(who).rxtype = 0;
      v.at(who).rxb4 += 1;
    }
  }
  ...
  if( H0 > 0 ){			// dynamic HIV - now turned on even when dyn isn't
    for( unsigned int ii = 0; ii < dweights.size(); ++ii ){//weights for prev
...
//* Prevalence of those on each of the separate regimens and number started on each treatment
    for(unsigned int ui = 0; ui < rez.ibmrx.size(); ++ui){
      rez.ibmrx[ui][runno].at(i) = (double)prev_rx.at(ui)/size; 
      rez.ibmrxcp[ui][runno].at(i) = (double)prev_rx_cp.at(ui)/size;    
    }
    rez.ibmnbcrx[runno].at(i) = nbc_rx; // base-case treatment
    rez.ibmnintrx[runno].at(i) = nint_rx; // intervention treatment
    rez.rxcat[runno].at(i) = catII; // number of catII treatments
    ...
    if( ntbdet > 0){
      rez.TBpropRetx[runno].at(i) = (double)ntbretx / ntbdet; //TB prop retx
    } else {
      rez.TBpropRetx[runno].at(i) = -1; //TB prop retx
    }
    if( ntbd > 0){
      rez.TBdeathavA[runno].at(i) = tbavaod / ntbd; //TB average age at death
    } else {
      rez.TBdeathavA[runno].at(i) = -1; //TB average age at death
    }
...
int population::ibmrun( const gsl_rng *r, ofstream& lgf, parameters& parz, results& rez, int runno){
  ...
  evk.insert(make_pair<string,int>("TBar",22));	  // -- TB after treatment ~activation w/o counts
  ...
  for(i = 0; i < parz.notime; ++i){//timeloop
    ...
    // events requiring setup_weights - called within
