...
P <- read.csv(file='shared/TBtargetsZM.csv')
load('shared/Ztargs.Rdata')
## HIV prevalence graph
## zth <- read.csv(file='shared/zmps.csv')# already corrected!
H <- read.csv(file='shared/AIDSinfoHIV.csv') #SA national
zth <- H[H$country=='ZM',c('year','hivp','hivplo','hivphi')]
zsat <- read.csv(file='shared/ZAMSTARtargetsZM.csv',header=F)
rownames(zsat) <- zsat[,1]
fs <- Ztargs$zshiv / zth[21,'hivp']
...
## TS2&6
axis(1);axis(2)
points(P$year,P$prev100k * Ztargs$zstbp/P$prev100k[n])
points(2010,Ztargs$zstbp,lwd=2,col='blue',cex=1.5);
lines(c(2010,2010),(1-0.5*c(-Ztargs$zstbpw,+Ztargs$zstbpw))*Ztargs$zstbp,lwd=2,col='blue') #todo
eps <- .1
points(2010-eps,Ztargs$zsrx,lwd=2,col='green',cex=1.5);
lines(c(2010,2010)-eps,(1-0.5*c(-Ztargs$zsrxw,+Ztargs$zsrxw))*Ztargs$zsrx,lwd=2,col='green') #todo
lines(P$year,P$prevhi * Ztargs$zstbp/P$prev100k[n], lty=2)
lines(P$year,P$prevlo * Ztargs$zstbp/P$prev100k[n], lty=2)
legend('topleft',fill=c('blue','green'),bty='n',legend=c('Undiagnosed TB','On TB treatment'))
dev.off()

## both!
## pdf(file='TS3.pdf')
## par(xaxs='i',yaxs='i',bty='l',las=1,mfrow=c(1,2))
## plot(x=c(1980.0,2010.5),y=c(0,2000),type='n',xlab='Year',
##      ylab='Incidence rate (per 100k per year)',
##      main="", axes=F )
## lines(tz,R4[,5])
## lines(tz,R2[,7]*R4[,2],col='red')
## axis(1);axis(2)
## points(P[,'year'],P[,'inc100k'],lwd=2)
## lines(P[,'year'],P[,'inclo'],lty=2,lwd=2)
## lines(P[,'year'],P[,'inchi'],lty=2,lwd=2)
## points(P[,'year'],P[,'tbhivinc100k'],lwd=1,col='red')
## lines(P[,'year'],P[,'tbhinclo'],lty=2,lwd=1,col='red')
## lines(P[,'year'],P[,'tbhinchi'],lty=2,lwd=1,col='red')
## plot(x=c(1980.0,2010.5),y=c(0,2500),type='n',xlab='Year',
##      ylab='Prevalence (per 100k)',
##      main="", axes=F )
## lines(tz,R4[,2])
## axis(1);axis(2)
## points(P[,'year'],P[,'prev100k'],lwd=2)
## lines(P[,'year'],P[,'prevlo'],lty=2,lwd=2)
## lines(P[,'year'],P[,'prevhi'],lty=2,lwd=2)
## dev.off()
...
pdf('ZZ3.pdf')
ytop <- 30
plot(c(1980,2010),c(0,ytop),xlab='year',ylab = 'HIV prevalence 18+ (%)',type='n',main='ZM ZAMSTAR')
...
pdf('ZZ4.pdf')
par(xaxs='i',yaxs='i',bty='l',las=1)
plot(x=c(1980.0,2010.5),y=c(0,10),type='n',xlab='Year',
     ylab='ARI (% per year)',
     main="", axes=F )
n <- dim(P)[1]
...
## 3,3
axis(1);axis(2)
points(2010,Ztargs$zsari,col='blue',cex=1.5,lwd=2)
lines(c(2010,2010),(1+c(Ztargs$zsariw,-Ztargs$zsariw))*Ztargs$zsari, lwd=2,col='blue')
dev.off()
